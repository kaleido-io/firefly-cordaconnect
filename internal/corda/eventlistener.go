...
// listenerCheckpoint is our Corda specific custom options that can be specified when creating a listener
type listenerOptions struct {
	Commands []string `json:"commands,omitempty"`
}

// listenerConfig is the configuration parsed from generic FFCAPI connector framework JSON, into our Corda specific options
type listenerConfig struct {
	name      string
	fromTime  fftypes.FFTime
...
// listenerCheckpoint is our Corda specific checkpoint structure
type listenerCheckpoint struct {
	Timestamp fftypes.FFTime `json:"timestamp,omitempty"`
}

func (cp *listenerCheckpoint) LessThan(b ffcapi.EventListenerCheckpoint) bool {
	bcp := b.(*listenerCheckpoint)
	return cp.Timestamp.Time().Before(*bcp.Timestamp.Time())
}
