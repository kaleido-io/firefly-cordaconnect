...
public class EventStream {
    ...
    // Ongoing batch thread should not compete with other tasks, so we have a
    // separate scheduler for the inFlightTask
    ...
    // It is canceled and rescheduled to execute after batchTimeout time, whenever
    // new batch of events begin processing
    ...
    // Acquired by a thread when it starts attempts with retries to push a batch to
    // the client
    // Released when a ack is received, or when we have SKIP error handling and an
    // error is received
    ...
    public void addStreamConsumer(WebSocketConnection connection) {
        log.info("{}, Adding stream consumer client with connection id {}", eventStreamInfo.getId(),
                connection.getId());
    ...
    private void _dispatchInQueueBatches() {
        // if there are batches in buffer waiting to be delivered, start dispatch if no
        // new batch is in process
        if (this.eventBatchBuffer.isEmpty()) {
            // cancel any running batch retries as we received the ack
            // when there are events in the buffer, next batch will cancel previous retry
            // task;
            cancelInFlightBatch();
        }
    ...
    public void onAck() {
        ...
        if (!eventBatchBuffer.isEmpty())
            eventBatchBuffer.remove();
    ...
    public void onError(String connectionId) {
        ...
        if (eventStreamInfo.getErrorHandling() == ErrorHandling.SKIP) {
            log.debug("{} SKIP error handler, releasing in flight lock to allow next batch push.",
                    eventStreamInfo.getId());
            if (!eventBatchBuffer.isEmpty())
                eventBatchBuffer.remove();
            inFlightLock.release();
            _dispatchInQueueBatches();
    ...
    public void onSuspend() {
        ...
        // subscriptions will take care of restarting from appropriate time based on
    ...
    private void onBatchTimeoutExpiry() {
        // batch timeout expired, we should push if push the new batch if we have some
        // events
        log.trace("{} batch timeout expired.", eventStreamInfo.getId());
        if (currentBatch.size() > 0) {
            log.debug("{} On batchTimeout expiry, pushing new batch with {} events, batch size {}",
                    eventStreamInfo.getId(), currentBatch.size(), eventStreamInfo.getBatchSize());
            onBatchComplete();
    ...
    /*
     * Used by Upstream subscription to push events as they arrive. New events are
     * ignored of stream is suspended.
     * Triggers the batch processing logic, i.e., adds the event to current batch or
     * start a batchtimeout timer if
     * starting a new batch or starts dispatch if current batch is complete. Blocks
     * if current batch is full and not
     * dispatched.
     */
    public void pushNewEvent(Event<?> vaultEvent) {
        ...
        if (currentBatch.size() == 0) {
            ...
            batchTimeoutTask = scheduler.schedule(() -> onBatchTimeoutExpiry(), eventStreamInfo.getBatchTimeoutMs(),
                    TimeUnit.MILLISECONDS);
        }

        if (currentBatch.size() < eventStreamInfo.getBatchSize() - 1) {
            log.debug("{} current batch size {}, batch size {}", eventStreamInfo.getId(), currentBatch.size() + 1,
                    eventStreamInfo.getBatchSize());
            currentBatch.add(vaultEvent);
        } else if (currentBatch.size() == eventStreamInfo.getBatchSize() - 1) {
            batchTimeoutTask.cancel(true);
            currentBatch.add(vaultEvent);
            log.debug("{} Batch completed. cancelling batchTimeoutTask, size {}", eventStreamInfo.getId(),
                    currentBatch.size());
            onBatchComplete();
        } else {
    ...
    /*
     * Attempts to send a batch from queued batches. Inflight lock ensures that only
     * one batch (front of queue) is in-flight at a time.
     * After acquiring inflight lock, we cancel old inflight task if it is running.
     * Update future checkpoints of subscriptions (values that will be checkpointed
     * when ack is received for this eventstream. The inflihgt lock is released upon
     * receipt of an ack or an error (if SKIP errorhandling) and front of batch
     * buffer is removed,
     * to allow next batch of event to be disptached. The inflight lock can also be
     * released if there are no batches to send or eventstream is suspended or
    ...
    private void dispatchBatch() {
        ...
        if (this.activeConnections.isEmpty()) {
            log.debug("{} Aborting dispatch of batch {}, no active consumers of the stream", eventStreamInfo.getId(),
                    batchCount.get() + 1);
            inFlightLock.release();
            return;
        }
    ...
    private void _updateCheckpoint(String subscriptionId, Instant time) {
        ...
        if (oldCheckpoint == null || time.isAfter(oldCheckpoint)) {
            log.info("{} updating checkpoint for the subscription {}, new {}, old {}", eventStreamInfo.getId(),
                    subscriptionId, time, oldCheckpoint);
            subscriptionCheckpoints.put(subscriptionId, time);
        }
    }

    /*
     * Next batch attempt clears previous inflight task. If there are no more
     * batches to send, this task can be queued on receipt of ack
     * or error (SKIP handling) to cancel inflight task. We simply release the lock
     * after cancelling the task to allow new batch of events.
     */
     ...
     * Schedules a periodic task (InflightTask) to send data to active consumers of
     * this eventstream,
     * Inflight task attempts to send data to active consumers after every
     * blockedRetryDelaySec.
     * Inflight task would be cancelled if ack/ error(SKIP handling) is received
     * from a consumer
     */
    private void attemptBatchWithRetry(List<Event<?>> batch, long batchCount) {
        ...
        inFlightTask = inFlightScheduler.scheduleWithFixedDelay(() -> {
            ...
            try {
                ...
                log.error("{}, error while sending batch {} to client {}", eventStreamInfo.getId(), batchCount,
                        connection.getId(), e);
            }
