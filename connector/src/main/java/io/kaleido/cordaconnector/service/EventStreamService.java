...
public class EventStreamService {
  ...
  private static final ExecutorService scheduler = Executors.newFixedThreadPool(1);
  @Autowired
  ...
  private void restoreStreamsFromDb() {
    ...
    // this is called by @PostConstruct, so we need to use transaction template
    TransactionTemplate tmpl = new TransactionTemplate(txManager);
    EventStreamService self = this;
    tmpl.execute(new TransactionCallbackWithoutResult() {
      @Override
      protected void doInTransactionWithoutResult(TransactionStatus status) {
        eventStreamRepository.findAllStream().forEach(self::_createOrUpdateStream);
      }
    });
  }
  ...
  private void restoreSubscriptionsFromDb() {
    ...
    tmpl.execute(new TransactionCallbackWithoutResult() {
      @Override
      protected void doInTransactionWithoutResult(TransactionStatus status) {
        subscriptionRepository.findAllSubscriptions().forEach(self::_createOrResetSubscription);
      }
    });
  }
  ...
  private void checkPointSubscription(Subscription sub) {
    ...
    try {
      if (lastCheckpoint == null
          || sub.getTimeHWM() != null && sub.getTimeHWM().isAfter(lastCheckpoint.toInstant())) {
  ...
  public void onWebSocketMessage(WebSocketConnection conn, ClientMessage message) {
    scheduler.submit(() -> {
      ...
      try {
        ...
        logger.error("Received message from client {} on topic {} for which eventstream doesn't exists",
            conn.getId(), topic, e);
        return;
      }
      switch (message.getType()) {
        case LISTEN:
          // add connection to eventstream, eventstream will push events to conn as they
          // are available
  ...
  private void _createOrUpdateStream(EventStreamInfo eventStreamInfo) {
    String streamId = eventStreamInfo.getId();
    EventStream eventStream = this.streams.get(streamId);
    if (eventStream == null) {
      eventStream = new EventStream(eventStreamInfo);
    } else {
      eventStream.setEventStreamInfo(eventStreamInfo);
    }
    this.streams.put(streamId, eventStream);
    this.topicToEventStreamId.put(eventStreamInfo.getWebsocketTopic(), streamId);
    logger.info("Eventstream {} initialized/updated", streamId);
  }

  private void _suspendStream(String eventstreamId) {
    EventStream eventStream = this.streams.get(eventstreamId);
    if (eventStream != null) {
      // when stream is suspended, checkpoint and pause all subscriptions that push
  ...
  private void _resumeStream(String eventstreamId) {
    ...
    if (eventStream != null) {
      ...
      this.subscriptions.forEachValue(5, subscription -> {
        if (subscription.getStream().getEventStreamInfo().getId().equalsIgnoreCase(eventstreamId))
          subscription.resumeSubscription();
      });
    }
  }
  ...
  @Transactional
  public EventStreamData createEventStream(EventStreamData streamData) {
    ...
    if (existingStream.isPresent()) {
      ...
      return new EventStreamData(createdStream);
    }
  }

  public List<EventStreamData> listEventStreams() {
    List<EventStreamInfo> eventStreams = eventStreamRepository.findAllStream().collect(Collectors.toList());
    return eventStreams.stream().map(EventStreamData::new).collect(Collectors.toList());
  }

  public EventStreamData getEventStreamById(String eventStreamId) {
    Optional<EventStreamInfo> existingStream = eventStreamRepository.findById(eventStreamId);
    EventStreamInfo eventStream = existingStream.orElseThrow();
    return new EventStreamData(eventStream);
  }

  @Transactional
  public EventStreamData updateEventStream(String eventStreamId, EventStreamData updatedStreamData) {
    Optional<EventStreamInfo> existingStream = eventStreamRepository.findById(eventStreamId);
    if (existingStream.isEmpty()) {
      ...
      return new EventStreamData(updatedStream);
    }
  }
  ...
  public boolean deleteEventStream(String eventStreamId) {
    ...
    if (existingStream.isEmpty()) {
      ...
      Stream<String> subscriptionsToDelete = subscriptionRepository
          .listSubscriptionIdsByEventStreamId(eventStreamId);
  ...
  private void _createOrResetSubscription(SubscriptionInfo subscriptionInfo) {
    String subId = subscriptionInfo.getId();
    Subscription subscription = this.subscriptions.get(subId);
    if (subscription == null) {
      // create request, create runtime
      logger.info("Creating subscription runtime for {}", subId);
    ...
    logger.info("Subscription runtime for {} initialized/reset", subId);
  }
